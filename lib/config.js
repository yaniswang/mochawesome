...
module.exports = function (options) {
  ...
  config.copyShotOnlyFail       = _getOption('copyShotOnlyFail', options, false);
  ...
  config.buildImgsDir  = path.join(config.buildDir, 'img');
  ...
  config.srcImgsDir    = path.join(config.srcDir, 'img');
  ...
  config.reportImgsDir = path.join(config.reportDir, 'img');
  config.reportCssDir   = path.join(config.reportDir, 'css');
  config.reportScreenshotsDir   = path.join(config.reportDir, 'screenshots');

  // Report Files
  config.reportJUnitFile = path.join(config.reportDir, _getOption('reportName', options) + '.xml');
  ...
  config.vendorJsFiles  = [path.join(config.nodeModulesDir, 'jquery', 'dist', 'jquery.js'),
                           ...
                           path.join(config.srcJsDir, 'lightbox.js'),
