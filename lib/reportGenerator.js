...
function generateReport (config) {
  ...
  async.auto({
      ...
      copyImgs: ['createDirs', function(callback) {
        // after creating directories,
        // copy fonts to fonts dir
        copyImgs(config, callback);
      }]
...
function createDirs (config, inline, callback) {
  ...
  if (!inline) {
    dirs = dirs.concat([config.reportJsDir, config.reportFontsDir, config.reportCssDir, config.reportScreenshotsDir]);
  }
...
function copyImgs (config, callback) {
  ncp(config.buildImgsDir, config.reportImgsDir, function (err) {
    if (err) callback(err);
    callback(null, 'done');
  });
}
